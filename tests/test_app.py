...
def test_every_plotly_chart_has_an_explicit_key_and_axes_are_locked():
    ...
    assert len(calls) == 6 and all(re.search(r'key="[a-z_]+"', c) for c in calls), calls
    assert len({re.search(r'key="([a-z_]+)"', c).group(1) for c in calls}) == 6            # jeder Schlüssel nur einmal
